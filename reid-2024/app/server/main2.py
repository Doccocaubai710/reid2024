...
from app.deeputils.bytetracker_noreid import BYTETracker
from app.utils import const
from app.utils.benchmark import BenchmarkMultiThread
# REMOVED: from app.utils.embeddings import extract_embedding
...
# REMOVED: extract_embeddings function - not needed anymore
...
class MainServer:
    def __init__(self):
        self.receiver = None
        self.video_writers = {}
        # REMOVED: self.storage = PersonIDsStorage()
        ...
        while True:
            try:
                ...
                if info.get("status") == "start":
                    ...
                    # Process detections - DIRECTLY extract bboxes and scores
                    bodys = get_face_body_from_detection(opencv_image, detections)
                    _time = time.perf_counter()
                    
                    # COMPLETELY REMOVED: extract_embeddings call
                    # Extract bboxes and scores DIRECTLY from detection data
                    boxes = []
                    confidences = []
                    ...
                    for body in bodys:
                        if body.get("frame").tolist() == []:
                            continue
                        boxes.append(body.get("bbox"))
                        confidences.append(body.get("score"))
                    
                    boxes = np.asarray(boxes) if boxes else np.array([]).reshape(0, 4)
                    confidences = np.asarray(confidences) if confidences else np.array([])
                    
                    console.print(f"Process detections time: {time.perf_counter() - _time}")
                    console.print(f"Found {len(boxes)} detections")
                    ...
                    # Pass None for features to bytetracker - PURE IoU TRACKING
                    bboxes, scores, ids = bytetrack.update(boxes, confidences, None)
                    tracking_ids = np.array(ids).astype(np.int32)
                    new_ids = np.setdiff1d(tracking_ids, tracked_ids)
                    if new_ids.size > 0:
                        logging.info(f"New tracking IDs: {tracking_ids}")
                        logging.info(f"New IDs this frame: {new_ids}")

                    # SIMPLIFIED tracking logic - no PersonID storage needed
                    actual_new_ids = new_ids.tolist() if new_ids.size > 0 else []

                    _time = time.perf_counter()
                    # Process tracking results - SIMPLIFIED
                    if len(bboxes) > 0:
                        console.print(f"Number of tracked objects: {len(tracking_ids)}")
                        console.print(f"Tracking scores: {scores}")
                        
                        tracked_ids = np.concatenate((tracked_ids, actual_new_ids))
                        logging.info(f"All tracked_ids: {tracked_ids}")
