...
class STrack(BaseTrack):
    ...
    def __init__(self,tlwh,score,body_feature,buffer_size=30):
        ...
        self.alpha=0.9
        self.total_frame = 0
        self.update_bodys(body_feature)
        
    def update_bodys(self,body_feature):
        self.total_frame += 1
        ...
        if self.smooth_body is None:
            ...
            self.smooth_body= body_feature*(self.alpha) + self.smooth_body*(1-self.alpha)
    ...
    def re_activate(self,new_track,frame_id,new_id=False):
        ...
        self.update_bodys(new_track.curr_body)
        ...
        if track_bodys is not None:
            ...
            if track_bodys.size > 0:  # Ensure track_bodys is not empty
                ...
                if track_bodys_tensor.ndim > 1:
                    track_body_feature = F.normalize(track_bodys_tensor, dim=1).cpu().numpy()
                else:
                    track_body_feature = F.normalize(track_bodys_tensor, dim=0).cpu().numpy()
            else:
            ...
            if len(boxes_keep)>0:
                detections = [STrack(tlwh,s,f) for (tlwh,s,f) in zip(boxes_keep,scores_keep,id_feature)]
            else:
                detections=[]
            ...
            for itracked, idet in matches:
                ...
                # Check if this track failed feature matching but succeeded with IoU
                if track.track_id in unmatched_feature_tracks:
                    # Get the original tlwh bounding box from track
                    tlwh = track.tlwh
                    
                    # Check if the track is near an edge
                    is_near_edge, edge_type = self.is_near_edge(tlwh)
                    
                    if is_near_edge:
                        # This is a strong ID switch candidate
                        if track.track_id not in self.switch_candidates:
                            self.switch_candidates[track.track_id] = []
                            
                        self.switch_candidates[track.track_id].append({
                            'frame': self.frame_id,
                            'edge': edge_type,
                            'feature_dist': 'high',  # Failed feature matching
                            'iou_dist': 'low'  # Succeeded IoU matching
                        })
                        
                        logging.info(f"ID switch detected: Track {track.track_id} at frame {self.frame_id} near {edge_type} edge")
            ...
            if len(boxes_second)>0:
                detections_second = [STrack(tlwh,s,f) for (tlwh,s,f) in zip(boxes_second,scores_second,id_feature_second)]
            else:
                detections_second=[]
