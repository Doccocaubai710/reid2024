from app.tracker.trackers.cmc import *
from app.tracker.trackers.utils import *
from app.tracker.trackers.track import *


class Tracker(object):
    def __init__(self, args, vid_name, use_cmc=False):  # Add use_cmc parameter
        # Initialize
        self.args = args
        self.max_time_lost = args.max_time_lost
        self.use_cmc = use_cmc  # Control CMC usage
        ...
        # Set global motion compensation model ONLY if enabled
        if self.use_cmc:
    ...
    def update(self, dets, dets_95):
        ...
        # Camera motion compensation - ONLY IF ENABLED
        if self.use_cmc and self.cmc is not None:
            warp_matrix = self.cmc.get_warp_matrix()
            apply_cmc(tracked_lost, warp_matrix)
            apply_cmc(new, warp_matrix)
        # If CMC is disabled, skip this step entirely
