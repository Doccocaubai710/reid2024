...
from app.offline_processing.id_switch_v3 import Tracklet, extract_id_switches_from_log, filter_nearby_switches
...
def r_matching_single_camera(tracklets, frame_shape, distance_matrix=None, valid_track_ids=None, similarity_threshold=0.2, err_threshold=30):
    ...
    final_clusters = match_remain(
        ...
        distance_matrix, valid_track_ids, similarity_threshold=0.23)
    ...
    if uncertain_tracklets:
        ...
        final_clusters = match_remain(
            uncertain_tracklets, [], final_clusters, distance_matrix, valid_track_ids, similarity_threshold=0.23)
    ...
    while made_progress and remaining_tracklets:
        ...
        for tracklet in list(remaining_tracklets):
            ...
            for cluster_id, cluster_tracklets in clusters.items():
                ...
                if avg_dist <= similarity_threshold and max_dist<=similarity_threshold:
...
def match_remain(unmatched_tracklets, short_tracklets, clusters, distance_matrix, valid_track_ids, similarity_threshold=0.23):
    """
    ...
    for cluster_id, cluster_tracklets in clusters.items():
        if cluster_id == 5:  # Skip specific clusters if needed
            continue
    ...
    # If you have the actual feature vectors for these tracklets, you could perform true re-ranking:
    # if all(hasattr(t, '_feature_vector') for t in valid_tracklets) and all(hasattr(t, '_feature_vector') for t in [t for cluster in clusters.values() for t in cluster]):
    #     query_features = np.array([t._feature_vector for t in valid_tracklets])
    #     gallery_features = np.array([t._feature_vector for t in [t for cluster in clusters.values() for t in cluster]])
    #     reranked_matrix = re_ranking(query_features, gallery_features, k1=10, k2=6, lambda_value=0.3)
    
    # Match tracklets based on distances
    matched_tracklets = []
...
def dist_based_re_ranking(original_dist, k1=20, k2=6, lambda_value=0.3):
    ...
    return final_dist
def split_tracklets_from_log(tracklets, log_file="/home/aidev/workspace/reid/Thesis/reid-2024/trash/2025-06-20 17-20-38-0286ef1065c64dae891c075d5514f9ea/example.log"):
    """
...
def process_session(session_dir, video_path=None):
    ...
    saved_images = extract_tracklet_mean_frames(
        ...
        video_path="/home/aidev/workspace/reid/Thesis/reid-2024/app/assets/videos/cam3.MOV",
        output_dir=os.path.join("/home/aidev/workspace/reid/Thesis/reid-2024/Screenshot", "mean_feature_frames_cam3"),
    ...
    logging.info("Saving cluster data for multi-camera matching...")
    ...
    logging.info(f"Saved matching data files: {list(saved_files.keys())}")
...
def main():
    ...
    parser.add_argument("--log_file", type=str, default="/home/aidev/workspace/reid/Thesis/reid-2024/trash/2025-06-20 17-20-38-0286ef1065c64dae891c075d5514f9ea/example.log",
