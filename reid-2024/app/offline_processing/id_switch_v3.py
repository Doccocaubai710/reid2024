...
class Tracklet(object):
    ...
    def get_best_detection_indices(self, all_tracklets=None,enable=False, max_overlap_threshold=0.2, max_area_count=30, final_count=7,frame_shape=(1080,1920)):
        ...
        1. Filter to minimize overlap with other tracklets' boxes in the same frame
        2. From those, select the largest boxes by area
        3. From those, select the highest confidence boxes
        ...
        Args:
            ...
            max_area_count: Number of largest boxes to consider after overlap filtering
            final_count: Final number of boxes to select by confidence
        ...
        def is_in_uncertain_zone(bbox, frame_shape):
            ...
            Returns:
                True if bbox is in uncertain zone (should be filtered out)
            """
        ...
        if all_tracklets and enable:
            ...
            # If no other tracklets provided, use zone-filtered indices
            low_overlap_indices = zone_filtered_indices
        
        # Filter boxes with overlap below threshold
        # low_overlap_indices = [idx for idx, score in overlap_scores if score < max_overlap_threshold]
        
        # # If no boxes with low overlap, use all boxes
        # if not low_overlap_indices:
        #     low_overlap_indices = list(range(len(self.bboxes)))
        #low_overlap_indices=zone_filtered_indices
        # Step 2: From low-overlap boxes, select largest by area
        area_with_index = [(i, (self.bboxes[i][2]-self.bboxes[i][0]) * (self.bboxes[i][3]-self.bboxes[i][1])) for i in low_overlap_indices]
        ...
        # Step 3: From largest boxes, select highest confidence
